##### description: Gene set methylation change trends in 90-day HDBR
...
#Load DNA methylation data of 90-days HDBR,nobatch_beta_HDBR.txt and SampleSheet.csv are available for download at https://www.spacelifescience.cn/search/ (search for nobatch_beta_HDBR, DNA methylation)
myCombat_90bed <- read.table("90-days HDBR/DNA methylation/nobatch_beta_HDBR.txt", header=T, row.names=1, as.is=T, sep="\t")
SampleSheet_90bed <- read.csv("90-days HDBR/DNA methylation/SampleSheet.csv")
table(SampleSheet_90bed$Sample_Group)

times <- factor(SampleSheet_90bed$Sample_Group, levels=c("P15","B30", "B60", "B90", "R30"))
times <- unique(times)
compare.group <- times[1:2]
DMP_res_90 <- champ.DMP(beta = myCombat_90bed,
                         pheno = as.character(SampleSheet_90bed$Sample_Group),
                         compare.group = compare.group,
                         adjPVal = 1,
                         adjust.method = "BH",
                         arraytype = "450K")
DMP_res_90_df <- DMP_res_90[[1]]
...
for(term in geneset_all_df_PPI$Description){
  ...
  interested_geneset_annotation <- DMP_res_90_df[DMP_res_90_df$gene%in%interested_geneset & DMP_res_90_df$feature%in%promoter_region, ]
  interested_geneset_meth_probe <-  as.data.frame(myCombat_90bed[rownames(interested_geneset_annotation), ])
  ...
  aov_p <- apply(interested_geneset_meth_mat, 1, aov_fun, group=SampleSheet_90bed$Sample_Group)
  f <- which(aov_p<0.05)
  ...
  if(nrow(filter_interested_geneset_meth_mat)>1){
     ...
     if(nrow(filter_interested_geneset_meth_mat)>1){
        mean_exp_mat <- apply(filter_interested_geneset_meth_mat, 1, function(x) tapply(x, factor(SampleSheet_90bed$Sample_Group, levels=c("P15","B30", "B60", "B90", "R30")), mean))
        mean_exp_mat <- t(mean_exp_mat)

        trend_p <- trend_plot(mean_exp_mat)

        plot_list_90bed <- c(plot_list_90bed, list(trend_p))
		term <- stringr::str_split_i(term, " - ", 1)
        names(plot_list_90bed)[length(plot_list_90bed)] <- term
     }

    }
}

saveRDS(plot_list_90bed, "Multi-group comparisons of geneset methy trend/result/plot_list_90bed.Rds")
for(i in 1:length(plot_list_90bed)){
  	filename<-paste("Multi-group comparisons of geneset methy trend/result/90bed_",names(plot_list_90bed)[i],".pdf",sep = "")
    pdf(filename,width=5,height=3)
	trend_p <- plot_list_90bed[i]
    print(trend_p)
    dev.off()
}
