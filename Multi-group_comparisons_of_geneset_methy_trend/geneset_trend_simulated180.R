##### description: Gene set methylation change trends in 180-day CELSS
...
myCombat_case <- read.table("180-days space simulation/DNA methylation/nobatch_beta_CELSS.txt", header=T, row.names=1, as.is=T, sep="\t")
SampleSheet180_case <- read_excel("180-days space simulation/DNA methylation/SampleSheet.xlsx")


SampleSheet180_case$Sample_Group <- factor(SampleSheet180_case$Sample_Group, levels=unique(SampleSheet180_case$Sample_Group))
SampleSheet180_case$time <- as.character(SampleSheet180_case$Sample_Group)
SampleSheet180_case$time[SampleSheet180_case$time%in%c("Pre45", "Pre15")] <- "Pre"
time<-unique(SampleSheet180_case$time)
SampleSheet180_case$time <- factor(SampleSheet180_case$time, levels=time)
sample_order <- order(SampleSheet180_case$Sample_Group)
myCombat_case <- myCombat_case[,sample_order]
SampleSheet180_case <- SampleSheet180_case[sample_order, ]

myCombat_case_agg <- apply(myCombat_case, 1, function(x) {y<-tapply(x[1:8], rep(1:4, 2), mean);return(c(y, x[9:48]))})
myCombat_case_agg <- t(myCombat_case_agg)
rownames(myCombat_case_agg) <- rownames(myCombat_case)
colnames(myCombat_case_agg) <- colnames(myCombat_case)[-c(1:4)]
colnames(myCombat_case_agg) <- gsub("Pre15","Pre",colnames(myCombat_case_agg))
SampleSheet180_case_agg <- SampleSheet180_case[SampleSheet180_case$Sample_Group!="Pre45",]


times <- factor(SampleSheet180_case_agg$time, levels=unique(SampleSheet180_case_agg$time))
times <- unique(times)
compare.group <- times[1:2]
DMP_res_180_case <- champ.DMP(beta = myCombat_case_agg,
                                pheno = as.character(SampleSheet180_case_agg$time),
                                compare.group = compare.group,
                                adjPVal = 1,
                                adjust.method = "BH",
                                arraytype = "450K")
  
DMP_res_180_case_df <- DMP_res_180_case[[1]]
 

plot_list_simulated180 <- c()
for(term in geneset_all_df_PPI$Description){
  ...
  interested_geneset_annotation <- DMP_res_180_case_df[DMP_res_180_case_df$gene%in%interested_geneset & DMP_res_180_case_df$feature%in%promoter_region, ]
  interested_geneset_meth_probe <-  as.data.frame(myCombat_case_agg[rownames(interested_geneset_annotation), ])
  ...
  aov_p <- apply(interested_geneset_meth_mat, 1, aov_fun, group=SampleSheet180_case_agg$time)
  f <- which(aov_p<0.05)
  ...
  if(nrow(filter_interested_geneset_meth_mat)>1){
     ...
     if(nrow(filter_interested_geneset_meth_mat)>1){
        mean_exp_mat <- apply(filter_interested_geneset_meth_mat, 1, function(x) tapply(x, factor(SampleSheet180_case_agg$time, levels=unique(SampleSheet180_case_agg$time)), mean))
        mean_exp_mat <- t(mean_exp_mat)

        trend_p <- trend_plot(mean_exp_mat)

        plot_list_simulated180 <- c(plot_list_simulated180, list(trend_p))
		term <- stringr::str_split_i(term, " - ", 1)
        names(plot_list_simulated180)[length(plot_list_simulated180)] <- term
     }
...
saveRDS(plot_list_simulated180, "Multi-group comparisons of geneset methy trend/result/plot_list_simulated180.Rds")
for(i in 1:length(plot_list_simulated180)){
    filename<-paste("Multi-group comparisons of geneset methy trend/result/simulated180_",names(plot_list_simulated180)[i],".pdf",sep = "")
    pdf(filename,width=5,height=3)
	trend_p <- plot_list_simulated180[i]
    print(trend_p)
    dev.off()
}
