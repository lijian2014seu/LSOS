...
library(ChAMP); library(ggvenn); library(ggplot2); library(openxlsx)
library(dplyr); library(tibble); library(stringr); library(data.table)
...
dataset_paths <- list(
  list(
    ...
    comparisons = list(c("T1", "T2"), c("T2", "T3"), c("T1", "T3"), c("T1", "D10_pro"), c("T2", "D10_pro"), c("D10_pro", "T3")),
    data_type = "450K",
    name1 = "M13"
  ),
  list(
...
for(exp in dataset_paths){
  ...
  for(comp in exp$comparisons){
    ...
    if (!is.null(down_probes)) { df[down_probes, "change"] <- "down" }
    table(df$change)
